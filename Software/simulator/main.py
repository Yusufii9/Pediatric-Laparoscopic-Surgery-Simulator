...
class GUI(object):
    ...
    def __init__(
        ...
        self.tip_low = tip_low
        self.tip_high = tip_high
        ...
        self.warning_time = [
            ...
            0,
            0,
        ]  # Timer variable array with an index for each different possible warning
        ...
        self.feedback_menu = np.zeros(
            [self.displayHeight, self.displayWidth, 3], np.uint8
        )

        # Startup screen to show sensors starting
        self.startup_screen = np.zeros(
            [self.displayHeight, self.displayWidth, 3], np.uint8
        )
        ...
        self.spike_times = [
            [] for _ in range(9)
        ]  # 2D array --> i,j where i = data index, j = times saved for that data index
        self.spike_values = [
            [] for _ in range(9)
        ]  # 2D array to save the data value at spike start for plotting purposes
        ...
        self.startup_counter = (
            16  # Counter variable for sensor startup countdown at program start
        )
...
def main():
    ...
    def start_sensors():
        try:
            GUI.ser = serial.Serial(
                "COM4", 38400
            )  # COM port may change depending on computer/devices being used
    ...
    def display_warning(frame):
        ...
        if cur_time - GUI.warning_time[0] < 1:
            cv2.putText(
                frame, "Too much force!", (500, 700), GUI.font, 1, (0, 0, 255), 2
            )
        if cur_time - GUI.warning_time[1] < 1:
            cv2.putText(
                frame, "Slow left pitch acc.", (25, 650), GUI.font, 1, (0, 0, 255), 2
            )
        if cur_time - GUI.warning_time[2] < 1:
            cv2.putText(
                frame, "Slow left yaw acc.", (25, 700), GUI.font, 1, (0, 0, 255), 2
            )
        if cur_time - GUI.warning_time[3] < 1:
            cv2.putText(
                frame, "Slow right pitch acc.", (925, 650), GUI.font, 1, (0, 0, 255), 2
            )
        if cur_time - GUI.warning_time[4] < 1:
            cv2.putText(
                frame, "Slow right yaw acc.", (925, 700), GUI.font, 1, (0, 0, 255), 2
            )
        if cur_time - GUI.warning_time[5] < 1:
            cv2.putText(
                frame, "Slow left surge acc.", (25, 600), GUI.font, 1, (0, 0, 255), 2
            )
        if cur_time - GUI.warning_time[6] < 1:
            cv2.putText(
                frame, "Slow left rotation acc.", (25, 550), GUI.font, 1, (0, 0, 255), 2
            )
        if cur_time - GUI.warning_time[7] < 1:
            cv2.putText(
                frame, "Slow right surge acc.", (900, 600), GUI.font, 1, (0, 0, 255), 2
            )
        if cur_time - GUI.warning_time[8] < 1:
            cv2.putText(
                frame,
                "Slow right rotation acc.",
                (900, 550),
                GUI.font,
                1,
                (0, 0, 255),
    ...
    def check_sensor_warnings(frame, data, max_force):
        ...
        if len(data.split("|")) == 33:  # Make sure array of proper length
            if (
                float(data_split[0]) > max_force
            ):  # If > max_force N of force, add warning (dependent on task)
    ...
    def plot_data():
        ...
        (l6,) = ax.plot(
            time, R_surge, visible=False, color="brown", label="Right Surge"
        )
        ...
        xpos = Slider(
            axpos, "Time", 0, len(time) - visible_range, valinit=0.0, valstep=0.1
        )
        ...
        def handle_click(label):
            ...
            lines[index].set_visible(
                not lines[index].get_visible()
            )  # Set visibility to be opposite of what it was set at
            ...
            for li in lines:
                if li.get_visible():
                    for j in range(0, len(GUI.spike_times[i])):
                        ax.text(
                            float(GUI.spike_times[i][j]),
                            float(GUI.spike_values[i][j]),
                            "!!!",
                        )
        ...
        for i in range(0, 9):
            ...
            for line in lines:
                if len(line.split("|")) == 10:
                    ...
                    vars = line.split("|")
                    if (
                        float(vars[i + 1]) > GUI.warn_thresholds[i] and not high
                    ):  # i+1 to skip time variable at index 0
                        high = True
                        GUI.spike_times[i].append(
                            j
                        )  # Save the time index of the spike start
                        GUI.spike_values[i].append(
                            vars[i + 1]
                        )  # Save the data value at spike start
    ...
    def evaluate_state():
        ...
        if GUI.image_state == 1:
            ...
            if ret == True:
                ...
                lower_tip_bound = None
                upper_tip_bound = None
                ...
                lower_tip_bound = GUI.tip_low
                upper_tip_bound = GUI.tip_high

                tipMask = cv2.inRange(frameHSV, lower_tip_bound, upper_tip_bound)
                contours2, _ = cv2.findContours(
                    tipMask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
                )
                ...
                for cnt2 in contours2:
                    ...
                    # contour_count = 0
                    if area > 90 and area < 300:
                        tip_count += 1
                        # (x, y, w, h) = cv2.boundingRect(cnt2)
                        # cv2.rectangle(frame, (x - 5, y - 5), (x + 5 + w, y + 5 + h), (0, 255, 0), 2)
                        ...
                        # cv2.putText(frame, str(area), (cX, cY), cv2.FONT_HERSHEY_`SIMPLEX, 1, (0, 255, 0), 2)

                        """if area > 250 or (tip_count > 1):
                            cv2.putText(frame, "open", (cX + 5, cY + 5), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)"""

                        for i in range(len(cX_list) - 1):
                            # cv2.putText(frame, str(abs(cX - cX_list[i])), (cX + 20, cY + 20), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
                            if (abs(cX - cX_list[i]) < 1000 and (tip_count > 1)) or (
                                area > 200
                            ):
                                cv2.putText(
                                    frame,
                        ...
                        for i in range(len(cY_list) - 1):
                            # cv2.putText(frame, str(abs(cY - cY_list[i])), (cX + 10, cY + 10), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
                            if (abs(cY - cY_list[i]) < 1000 and (tip_count > 1)) or (
                                area > 200
                            ):
                                cv2.putText(
                                    frame,
                ...
                GUI.file.write(
                    str(round(time.time() - GUI.task_start, 3))
                    + "|"
                    + stuff_string.rstrip()
                    + "\n"
                )
                ...
                check_sensor_warnings(
                    frame, stuff_string.rstrip(), GUI.warn_thresholds[0]
                )  # Sending force threshold here incase we want to change depending on task
                ...
                cv2.putText(
                    frame,
                    str(round(time.time() - GUI.task_start, 2)),
                    (600, 35),
                    GUI.font,
                    1,
                    (0, 0, 255),
            ...
            # Write sensor data to file with time since task start in seconds
            GUI.file.write(
                str(time.time() - GUI.task_start) + "|" + stuff_string.rstrip() + "\n"
            )
            ...
            GUI.feedback_menu = np.zeros(
                [GUI.displayHeight, GUI.displayWidth, 3], np.uint8
            )
            cv2.putText(
                GUI.feedback_menu, "Main Menu", (1100, 35), GUI.font, 1, (0, 0, 255), 2
            )
            cv2.putText(
                GUI.feedback_menu,
                "Watch Previous Attempt",
                (880, 685),
                GUI.font,
                1,
                (0, 0, 255),
            ...
            cv2.putText(
                GUI.feedback_menu,
                "Force: " + str(len(GUI.spike_times[0])),
                (25, 170),
                GUI.font,
                1,
                (0, 255, 0),
            ...
            cv2.putText(
                GUI.feedback_menu,
                "Left pitch acc: " + str(len(GUI.spike_times[1])),
                (25, 215),
                GUI.font,
                1,
                (0, 255, 0),
            ...
            cv2.putText(
                GUI.feedback_menu,
                "Left yaw acc: " + str(len(GUI.spike_times[2])),
                (25, 260),
                GUI.font,
                1,
                (0, 255, 0),
            ...
            cv2.putText(
                GUI.feedback_menu,
                "Right pitch acc: " + str(len(GUI.spike_times[3])),
                (25, 305),
                GUI.font,
                1,
                (0, 255, 0),
            ...
            cv2.putText(
                GUI.feedback_menu,
                "Right yaw acc: " + str(len(GUI.spike_times[4])),
                (25, 350),
                GUI.font,
                1,
                (0, 255, 0),
            ...
            cv2.putText(
                GUI.feedback_menu,
                "Left surge acc: " + str(len(GUI.spike_times[5])),
                (25, 395),
                GUI.font,
                1,
                (0, 255, 0),
            ...
            cv2.putText(
                GUI.feedback_menu,
                "Left roll acc: " + str(len(GUI.spike_times[6])),
                (25, 440),
                GUI.font,
                1,
                (0, 255, 0),
            ...
            cv2.putText(
                GUI.feedback_menu,
                "Right surge acc: " + str(len(GUI.spike_times[7])),
                (25, 485),
                GUI.font,
                1,
                (0, 255, 0),
            ...
            cv2.putText(
                GUI.feedback_menu,
                "Right roll acc: " + str(len(GUI.spike_times[8])),
                (25, 530),
                GUI.font,
                1,
                (0, 255, 0),
            ...
            cv2.putText(
                GUI.main_menu,
                "Pediatric Laparoscopic Training Simulator",
                (320, 360),
                GUI.font,
                1,
                (0, 0, 255),
            ...
            cv2.putText(
                GUI.main_menu, "Ring Task", (25, 35), GUI.font, 1, (0, 0, 255), 2
            )
            cv2.putText(
                GUI.main_menu, "Suturing Task", (1030, 35), GUI.font, 1, (0, 0, 255), 2
            )
            cv2.putText(
                GUI.main_menu, "View Feedback", (1030, 685), GUI.font, 1, (0, 0, 255), 2
            )
            cv2.putText(GUI.main_menu, "Quit", (25, 685), GUI.font, 1, (0, 0, 255), 2)
            cv2.imshow(GUI.windowName, GUI.main_menu)
        elif GUI.image_state == 5:
            GUI.startup_screen = np.zeros(
                [GUI.displayHeight, GUI.displayWidth, 3], np.uint8
            )  # Clear text from window
            cv2.putText(
                GUI.startup_screen,
                "Starting sensors, please wait "
                + str(GUI.startup_counter)
                + " seconds...",
                (320, 360),
                GUI.font,
                1,
    ...
    def mouse_event(event, x, y, flags, param):
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            # Ring Task options
                ...
                if (
                    y < GUI.displayHeight / 2 and x > GUI.displayWidth / 2
                ):  # Top Right click
                    GUI.file.close()
    ...
    while True:
        ...
        key = cv2.waitKey(1)
        if (
            key == ord("q") or GUI.image_state == -1
        ):  # state -1 will tell program to quit
            quit_program()
            break
