...
class Task2PerformanceAnalyzer:
    ...
    def align_data(self):
        ...
        for signal in self.signal_names:
            ...
            aligned_data[f'Ref_task2_{signal}'] = aligned_ref
            aligned_data[f'User_task2_{signal}'] = aligned_user

        df_aligned = pd.DataFrame.from_dict({k: pd.Series(v) for k, v in aligned_data.items()})
        df_aligned.to_csv('aligned_signals_task2.csv', index=True)
        return df_aligned
    ...
    def map_aligned_timestamps_to_original(self):
        ...
        aligned_data = pd.read_csv('aligned_signals_task2.csv')
        ...
        for signal in self.signal_names:
            ...
            original_timestamps[f'Ref_task2_{signal}_Time'] = pd.Series(ref_times[:min_len])
            original_timestamps[f'User_task2_{signal}_Time'] = pd.Series(user_times[:min_len])
    ...
    def normalize_and_process_windows(self, window_size_seconds=5):
        ...
        for signal in self.signal_names:
            data[f'Ref_task2_{signal}'] = data[f'Ref_task2_{signal}'].apply(self.string_to_float)
            data[f'User_task2_{signal}'] = data[f'User_task2_{signal}'].apply(self.string_to_float)

        user_exclude_column = 'User_task2_Time'
        ref_exclude_column = 'Ref_task2_Time'
        ...
        self.normalized_data.dropna().to_csv("normalized_data_task2.csv")
        ...
        self.get_inf_df.to_csv("infoTest_task2.csv")
        ...
        print(f"Task 2 Mean DTW Distance: {mean_dtw}")
        print(f"Task 2 Max DTW Distance: {max_dtw}")
        print(f"Task 2 Min DTW Distance: {min_dtw}")
        print(f"Task 2 Standard Deviation: {std_dtw}")
    ...
    def process_videos(self):
        ...
        clip_time_boundaries.to_csv("task2_clip_time_boundaries.csv")
        self.info_df.to_csv("task2_weak_signal_performance.csv")
        ...
        grouped_clip_time_boundaries.to_csv("task2_grouped_clip_time_boundaries.csv")

        print(f"Task 2 Ref start timestamps: {grouped_clip_time_boundaries['Ref Start Time'].tolist()}")
        print(f"Task 2 Ref end timestamps: {grouped_clip_time_boundaries['Ref End Time'].tolist()}")
        print(f"Task 2 User start timestamps: {grouped_clip_time_boundaries['User Start Time'].tolist()}")
        print(f"Task 2 User end timestamps: {grouped_clip_time_boundaries['User End Time'].tolist()}")
        print(f"Task 2 Number of proposed combined clips: {len(grouped_clip_time_boundaries)}")
        ...
        for index, row in grouped_clip_time_boundaries.iterrows():
            ...
            if (ref_start_seconds != ref_end_seconds) and (user_start_seconds != user_end_seconds):
                user_target_name = f"Task2_Group{index}_User_Clip.mp4"
                ...
                ref_target_name = f"Task2_Group{index}_Ref_Clip.mp4"
                ...
                final_clip_name = f"Task 2 Feedback clips/Task2_Final_Clip_{index}.mp4"
                final_clip.write_videofile(final_clip_name)

        directory = 'Task 2 Feedback clips'
        mp4_files = [f for f in os.listdir(directory) if f.endswith('.mp4')]
        print(f"Task 2 Number of actual combined clips: {len(mp4_files)}")
