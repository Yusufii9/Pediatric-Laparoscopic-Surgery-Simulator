...
class Task1PerformanceAnalyzer:
    ...
    def normalize_and_process_windows(self, window_size_seconds=5):
        ...
        self.get_inf_df = pd.DataFrame(self.dtw_distances,
                                  columns=['Start Index', 'End Index', 'User Signal', 'Ref Signal', 'DTW Distance'])
    ...
    def process_videos(self):
        ...
        for i in range(len(self.get_inf_df)):
            if self.get_inf_df["DTW Distance"][i] > self.far_away[0]:
                ref_start_time = read_normalized_data["Ref_task1_Time"].loc[self.get_inf_df.loc[i][0]]
                ref_end_time = read_normalized_data["Ref_task1_Time"].loc[self.get_inf_df.loc[i][1]]
        ...
        for i in range(min(len(user_start_timestamps), len(user_end_timestamps))):
            ...
            break  # break for now since the videos are sped up, won't be able to slice them properly,
            # and would cause an error

        for i in range(min(len(ref_start_timestamps), len(ref_end_timestamps))):
            ref_start_seconds = ref_start_timestamps[i]
            ref_end_seconds = ref_end_timestamps[i]
            ref_target_name = f"Group{i}_Peaks_in_Video10.mp4"
        ...
        ref_text = TextClip("Expert", font="Amiri-bold", fontsize=50, color='white').set_duration(ref_clip.duration)\
            .margin(top=40, right=20, opacity=0).set_position(("right", "top"))

        user_clip_with_text = CompositeVideoClip([user_clip, user_text])
        ref_clip_with_text = CompositeVideoClip([ref_clip, ref_text])

        clips.append([user_clip_with_text, ref_clip_with_text])

        combined_videos = clips_array(clips)
        combined_videos.write_videofile("test_task1.mp4")


if __name__ == "__main__":
    analysis = Task1PerformanceAnalyzer('Video10_Reference_Seg1.csv', 'Video9_user_Seg1.csv', 'Video10.mp4',
                                        'Video9.mp4')
    aligned_data = analysis.align_data()
